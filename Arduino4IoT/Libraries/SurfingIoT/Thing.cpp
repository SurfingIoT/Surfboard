...
void changeModeViaButton() {
  if(millis()-lastDebounce>debounceDelay && ntime++>=100) {
    things.lastMode=things.currentMode;
    things.currentMode = things.currentMode==things.modeCounter ? 0 : things.currentMode + 1;
    things.modeChanged1=1;	
    ntime=0;
    lastDebounce=millis();
  }
}
...
void Thing::serialServer() {
  ...
  if(counter>0) {
     ...
     else if(command[0]=='>') { //debug info, ignore!
        return;
     }
...
void Thing::start(int intNumber) {
  if(intNumber>-1){
    digitalWrite((intNumber+2), HIGH); //PULL-UP INTx
